...
public class UserMarksActivity extends AppCompatActivity {
    ...
    // В UserMarksActivity.java обновляем метод setupListClickListeners
    private void setupListClickListeners() {
        publicMarksListView.setOnItemClickListener((parent, view, position, id) -> {
            ...
            openMapWithMarker(selectedMarker);
        });
    ...
    private void openMapWithMarker(Marker marker) {
        Intent mapIntent = new Intent(UserMarksActivity.this, MapMarksActivity.class);
        mapIntent.putExtra("marker", marker); // Передаем весь объект Marker
